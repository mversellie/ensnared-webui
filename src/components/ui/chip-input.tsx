...
export const ChipInput = ({ value, onChange, placeholder, className }: ChipInputProps) => {
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <div className="flex flex-wrap items-center gap-2 min-h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
        {value.map((chip, index) => (
          <Badge key={index} variant="secondary" className="pr-1 shrink-0">
            {chip}
            <button
              type="button"
